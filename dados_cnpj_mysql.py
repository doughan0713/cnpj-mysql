    ...
    if os.path.exists(filepath):
        with open(filepath, "r") as f:
            for line in f:
                ...
                os.environ[key.strip()] = value.strip().strip("'").strip('"').split(' #')[0].strip()
...
db_port = int(port) if port and port.isdigit() else (3306 if tipo_banco == 'mysql' else 5432)
if tipo_banco == 'mysql':
    engine_url = sqlalchemy.engine.URL.create(drivername="mysql+pymysql", username=username, password=password, host=host, port=db_port, database=dbname)
elif tipo_banco == 'postgres':
    engine_url = sqlalchemy.engine.URL.create(drivername="postgresql", username=username, password=password, host=host, port=db_port, database=dbname)
else:
    print('tipo de banco de dados não informado'); sys.exit()
engine = sqlalchemy.create_engine(engine_url).connect()
