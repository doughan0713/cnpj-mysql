...
if tipo_banco == 'mysql':
    ...
    print(f'tipo de banco de dados "{tipo_banco}" não suportado. Use "mysql" ou "postgres".')
    sys.exit()
...
def carregaTabelaCodigo(extensaoArquivo, nomeTabela):
    ...
    dtab.to_sql(nomeTabela, engine_, if_exists='append', index=None)
    # Index names are from a trusted list in the script, but we use a simpler way if possible.
    # identifiers like index names cannot be parameterized.
...
def carregaTipoPandas(nome_tabela, tipo, colunas):
    ...
    for arq in arquivos:
        ...
        #engine.execute('Drop table if exists estabelecimento')
        print('to_sql...', time.asctime())
        df.to_sql(nome_tabela, engine_, index=None, if_exists='append',method='multi',
