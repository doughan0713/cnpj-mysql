...
def carregar_env(caminho_env=".env"):
    """
    Carrega as variáveis de ambiente a partir de um arquivo .env personalizado.
    Retorna um dicionário com os pares chave/valor ou os valores padrão.
    Chaves são limpas de espaços em branco e aspas das extremidades.
    Este método evita o uso de dependências externas como python-dotenv.
    """
    env_vars = {}
        ...
        with open(caminho_env, "r", encoding="utf-8") as f:
            for linha in f:
                ...
                if not linha or linha.startswith("#"):
                    continue
                if "=" in linha:
                    # Dividir apenas na primeira ocorrência de '=' para suportar valores contendo '='
...
# Carrega parâmetros do servidor com fallback seguro.
env = carregar_env()

tipo_banco = os.environ.get("DB_TYPE") or env.get("DB_TYPE") or "postgres"
dbname = os.environ.get("DB_NAME") or env.get("DB_NAME") or "cnpj"
username = os.environ.get("DB_USER") or env.get("DB_USER") or "root"
password = os.environ.get("DB_PASS") or env.get("DB_PASS") or ""
host = os.environ.get("DB_HOST") or env.get("DB_HOST") or "127.0.0.1"

db_port_raw = os.environ.get("DB_PORT") or env.get("DB_PORT")
if db_port_raw and db_port_raw.isdigit():
    db_port = int(db_port_raw)
else:
    db_port = 3306 if tipo_banco == "mysql" else 5432
...
resp = input(f'Isto irá CRIAR TABELAS ou REESCREVER TABELAS no database {dbname.upper()} no servidor {tipo_banco} {host}:{db_port} e MODIFICAR a pasta {pasta_saida}. Deseja prosseguir? (S/N)?')
if not resp or resp.upper()!='S':
    sys.exit()

# Uso de sqlalchemy.engine.URL.create para criar de forma segura a URL de conexão,
# evitando injeção de parâmetros ou problemas com caracteres especiais nas credenciais.
...
# Prevenção contra injeção de SQL utilizando parametrização segura para inserção de metadados
engine.execute(
    text("insert into _referencia (referencia, valor) values (:ref, :val)"),
    {"ref": "CNPJ", "val": str(dataReferencia)}
)
engine.execute(
    text("insert into _referencia (referencia, valor) values (:ref, :val)"),
