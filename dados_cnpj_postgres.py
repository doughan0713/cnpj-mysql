...
# SECURITY: Use parameterized queries with bind parameters to prevent SQL injection vulnerabilities
engine.execute(text("insert into _referencia (referencia, valor) values ('CNPJ', :val)"), {"val": str(dataReferencia)})
engine.execute(text("insert into _referencia (referencia, valor) values ('cnpj_qtde', :val)"), {"val": str(qtde_cnpjs)})
