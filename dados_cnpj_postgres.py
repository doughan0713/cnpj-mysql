...
#Para mysql, requer biblioteca pymysql or mysqlclient (msqldb), desempenho semelhantes.
pip install pymysql
...
# Uso de sqlalchemy.engine.URL.create para construir a URL de conexão de forma segura,
# evitando injeção de credenciais e lidando corretamente com caracteres especiais.
if tipo_banco == 'mysql':
...
def carregaTabelaCodigo(extensaoArquivo, nomeTabela):
    ...
    # DDL statements (like CREATE INDEX) use interpolation for identifiers because they cannot be parameterized.
    # The index name is derived from internal trusted mapping and the table name is from a controlled list.
...
# Securely insert metadata using parameterized queries to prevent SQL injection.
engine.execute(
    text("insert into _referencia (referencia, valor) values (:referencia, :valor)"),
    {"referencia": "CNPJ", "valor": dataReferencia}
)
engine.execute(
    text("insert into _referencia (referencia, valor) values (:referencia, :valor)"),
